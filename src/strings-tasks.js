 ...
 *
 * @example:
 * split converts string into array, sort sorts it alphabetically
 * and join converts it back to a string
 ...
 *   countVowels('aEiOu') => 5
 *   countVowels('XYZ') => 1
 * if condition checks if the char is included in the vowel string and counts.
 */
...
function isPalindrome(str) {
  ...
  let revstr = '';
  for (let i = normalStr.length - 1; i >= 0; i -= 1) {
    revstr += normalStr[i];
  }
  return revstr === normalStr;
}
...
function findLongestWord(sentence) {
  ...
  let maxWord = '';
  words.forEach((helper) => {
    if (helper.length > maxWord.length) {
      maxWord = helper;
    }
  });
  return maxWord;
